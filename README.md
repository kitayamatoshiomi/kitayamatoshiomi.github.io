...
| 位置 | 中身 |
|---|---|
| 中央 | 白い霧と星。ゆっくり漂い、ドラッグで3D回転。星は白く、押すと色が付く（クリックでツールを開く） |
...
操作：ドラッグ＝3D回転（裏側まで）／ホイール＝拡大縮小／Shift＋ドラッグ＝平行移動。
キー：**R**＝球体表示と霧の切り替え（Obsidian のグラフビュー風）、**F**＝全画面、**/**＝入力欄へ。
全画面は上部の ⛶ ボタン、F キー、背景ダブルクリックでも切り替わります。
対話窓のコマンド：星の名前（光る）／「〜を開いて」／球体／リセット（視点）／色リセット（星を白に戻す）。
押した星の色と表示モードは、その端末の localStorage に記憶します（`wb-lit` / `wb-sphere`）。
